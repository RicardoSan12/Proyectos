...
export const updateTabBgPosition = () => {
	const tabActive = document.querySelector<HTMLButtonElement>('.filter-tab-active')
	const tabBg = document.querySelector<HTMLDivElement>('.filters__tab-bg');
	if (!tabActive || !tabBg) return
...
const updateActiveTab = (target:HTMLElement) => {
	...
	const activedTabContent = document.querySelector<HTMLElement>('.filters__active');
	const targetTab = target;
	const targetTabContent = target.dataset.target && document.querySelector<HTMLElement>(target.dataset.target);

	if(!activedTab || !activedTabContent || !targetTabContent) return
...
export const handleClick:EventListener = ({ target }) => {
	...
	if(targetElement && targetElement.hasAttribute('data-target')) updateActiveTab(targetElement);
};
